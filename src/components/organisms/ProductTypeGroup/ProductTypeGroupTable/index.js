...
const Render = ({ productTypeGroupList, loading, onOpenUpdate, onDelete }) => (
  <TableModule
    selectKey="productTypeGroupId"
    loading={loading}
    headCells={headCells}
    dataSources={productTypeGroupList}
    row={TableRowModule}
...
export default function ProductTypeGroupTable() {
  const selector = useSelector(
    ({
    ...
    }) => ({ productTypeGroupList, loading, page, totalPages, filters }),
    shallowEqual
  );
  ...
  const renderProps = {
    ...selector,
    onOpenUpdate: productTypeGroupId =>
      dispatch(getUpdateAction(productTypeGroupId)),
    onDelete: productTypeGroupId => dispatch(doDelete(productTypeGroupId))
  };
